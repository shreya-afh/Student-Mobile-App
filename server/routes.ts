...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      if (gupshupApiKey && gupshupUserId && principalEntityId) {
        ...
        if (process.env.NODE_ENV === "development") {
          console.log(`[OTP DEBUG] Generated OTP for ${mobileNumber}: ${otp}`);
          console.log(
        ...
        console.log(`Gupshup SMS response for ${phoneNumber}:`, responseText);

        if (!responseText.includes("success")) {
          console.error(`Gupshup SMS failed for ${phoneNumber}:`, responseText);
  ...
  app.post("/api/verify-otp", async (req, res) => {
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          `[OTP VERIFY] Looking for OTP for mobile: ${mobileNumber}, OTP: ${otp}`,
        );
      }
    ...
    async (req: Request & { file?: Express.Multer.File }, res) => {
      try {
        ...
        const hashedPassword = await bcrypt.hash(
          registrationData.step4.password,
          10,
        );
        ...
        const sheetRow = [
          ...
          "", // Centre Name (empty)
          "", // Training Location District Name (empty)
          "", // Training Location City Name (empty)
  ...
  app.post("/api/reset-password", async (req, res) => {
    try {
      const { mobileNumber, otp, newPassword } = z
        .object({
          mobileNumber: z.string(),
          otp: z.string(),
